export const SYSTEM_PROMPT = `You are the lead stylist at Glam Me Up, an elegant styling studio that prepares women for specific occasions (weddings, galas, dates, interviews, holidays, and more).
...
export function buildUserContent({ occasion, context, photos }) {
  const details = [];
...
export class StylistRequestError extends Error {
...
export function validateAnalyzeBody(body) {
  const { occasion, context, photos } = body || {};
  if (!occasion || !Array.isArray(photos) || photos.length < 3) {
    throw new StylistRequestError(400, "Please provide an occasion and at least 3 photos.");
  }
  if (photos.length > 5) {
    throw new StylistRequestError(400, "Please provide at most 5 photos.");
  }
  return { occasion, context: context || {}, photos };
}

export async function runStylistAnalysis(anthropic, model, { occasion, context, photos }) {
  const message = await anthropic.messages.create({
    model,
    max_tokens: 2200,
    system: SYSTEM_PROMPT,
    messages: [{ role: "user", content: buildUserContent({ occasion, context, photos }) }],
  });
